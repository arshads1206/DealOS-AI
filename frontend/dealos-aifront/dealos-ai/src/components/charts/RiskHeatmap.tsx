...
const cellTone: Record<(typeof severities)[number], (count: number) => string> = {
  ...
  critical: (n) => (n === 0 ? "bg-white/[0.02]" : "bg-[var(--color-crimson)]/60 shadow-[0_0_16px_-4px_rgba(207,87,81,0.6)]"),
};
            ...
            {severities.map((s) => {
              ...
              return (
                <div
                  key={s}
                  className={cn(
                    "h-9 rounded-md flex items-center justify-center font-mono text-xs tabular-nums transition-all duration-200 ease-out hover:scale-[1.06]",
                    cellTone[s](count),
