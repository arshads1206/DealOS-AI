          ...
          <Reveal key={r.id} delay={Math.min(i * 0.05, 0.2)}>
            <div onClick={() => setSelectedReportId(r.id)} className="h-full cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-gold-500)] rounded-xl text-left">
              <GlassCard className="p-5 h-full flex flex-col" interactive>
                ...
                <div onClick={(e) => e.stopPropagation()} className="mt-2 inline-block w-fit">
                  <Link to={`/companies/${r.companyId}`} className="text-xs text-[var(--color-gold-400)] hover:text-[var(--color-gold-300)]">
