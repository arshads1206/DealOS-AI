        ...
        <GlassCard className="p-6" raised>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={company.history} margin={{ top: 8, right: 8, left: -12, bottom: 0 }} barGap={4}>
              ...
              <Tooltip
                contentStyle={{ background: "#17181d", border: "1px solid rgba(201,162,39,0.24)", borderRadius: 10, fontSize: 12, fontFamily: "IBM Plex Mono" }}
                labelStyle={{ color: "#A6A6AE" }}
                cursor={{ fill: "rgba(245,243,238,0.03)" }}
              />
              <Legend wrapperStyle={{ fontSize: 12, fontFamily: "IBM Plex Sans", color: "var(--color-ink-3)" }} />
              <Bar dataKey="revenue" name="Revenue" fill="#C9A227" radius={[4, 4, 0, 0]} />
              <Bar dataKey="ebitda" name="EBITDA" fill="#5B9BD5" radius={[4, 4, 0, 0]} />
              <Bar dataKey="netIncome" name="Net Income" fill="#34B27F" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </GlassCard>
      </Reveal>
...
function KpiTile({ label, value, sub, tone }: { label: string; value: string; sub: string; tone?: "emerald" | "crimson" }) {
  return (
    <GlassCard className="p-4" interactive>
      ...
      <p className="font-mono text-xl font-semibold text-[var(--color-ink-0)] mt-2 tabular-nums">{value}</p>
