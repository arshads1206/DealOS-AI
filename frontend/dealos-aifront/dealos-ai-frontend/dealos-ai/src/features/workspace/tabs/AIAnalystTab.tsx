    ...
    <div className="grid grid-cols-1 xl:grid-cols-[220px_1fr_320px] gap-4 items-start">
              ...
              >
                ...
                <div className={cn("max-w-[80%] space-y-2", m.role === "user" && "flex flex-col items-end")}>
                  <button
                    ...
                    className={cn(
                      "rounded-2xl px-4 py-2.5 text-[13.5px] leading-relaxed text-left transition-colors",
                      m.role === "assistant"
                        ? cn(
                            "bg-white/[0.03] border text-[var(--color-ink-1)]",
                            selectedMsgId === m.id ? "border-[var(--color-gold-500)]/40" : "border-[var(--color-border-hairline)] hover:border-[var(--color-border-strong)]"
                          )
                        : "bg-gradient-to-b from-[var(--color-gold-300)] to-[var(--color-gold-500)] text-[#141008] font-medium"
                    )}
                  >
                    {m.content}
                  </button>
      ...
      <GlassCard className="p-5 hidden xl:block sticky top-24 max-h-[calc(100vh-260px)] overflow-y-auto" raised>
        ...
        {!selectedMsg || selectedMsg.role !== "assistant" ? (
          <p className="text-xs text-[var(--color-ink-3)]">Select an answer in the transcript to inspect its sources and reasoning trace.</p>
        ) : (
            ...
            {selectedMsg.citations && selectedMsg.citations.length > 0 && (
              <div>
                ...
                <div className="space-y-2.5">
                  {selectedMsg.citations.map((c) => (
                    <button key={c.id} className="w-full text-left rounded-lg border border-[var(--color-border-hairline)] bg-white/[0.02] p-3 hover:border-[var(--color-border-strong)] transition-colors group">
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <ChevronRight size={12} className="text-[var(--color-ink-4)] group-hover:text-[var(--color-gold-400)] transition-colors shrink-0" />
                      </div>
